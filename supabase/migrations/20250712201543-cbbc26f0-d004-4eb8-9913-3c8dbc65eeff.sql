...
SELECT 
  ...
  SUM(COALESCE(i.current_value, i.total_amount)) as current_value,
  SUM(i.token_amount) as total_tokens,
  AVG(COALESCE(i.roi_percentage, 0)) as avg_roi,
  MAX(i.created_at) as last_investment_date,
  -- Growth metrics
  SUM(COALESCE(i.current_value, i.total_amount) - i.total_amount) as total_gains,
  CASE 
    WHEN SUM(i.total_amount) > 0 THEN 
      (SUM(COALESCE(i.current_value, i.total_amount) - i.total_amount) / SUM(i.total_amount)) * 100
    ELSE 0 
  ...
  -- Risk metrics
  STDDEV(COALESCE(i.roi_percentage, 0)) as roi_volatility
...
SELECT 
  ...
  COALESCE(SUM(i.token_amount), 0) as tokens_issued,
  COALESCE(p.total_tokens, 1000) - COALESCE(SUM(i.token_amount), 0) as available_tokens,
  -- Performance metrics
  ...
  RANK() OVER (PARTITION BY p.city ORDER BY COALESCE(SUM(i.total_amount), 0) DESC) as investment_volume_rank,
  -- Funding progress
  CASE 
    WHEN p.funding_target > 0 THEN (COALESCE(SUM(i.total_amount), 0) / p.funding_target) * 100
    ELSE 0 
...
-- Create secure views with RLS for user access
CREATE VIEW public.user_portfolio_performance AS
SELECT * FROM public.portfolio_performance_mv
WHERE user_id = auth.uid();

CREATE VIEW public.public_property_insights AS
SELECT * FROM public.property_market_insights_mv;

CREATE VIEW public.public_investment_opportunities AS
SELECT * FROM public.investment_opportunities_mv;
...
GRANT SELECT ON public.user_portfolio_performance TO authenticated;
GRANT SELECT ON public.public_property_insights TO authenticated;
GRANT SELECT ON public.public_investment_opportunities TO authenticated;